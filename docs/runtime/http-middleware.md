...
- 成功后：`c.Set("jwt_subject", claims.Subject)`、`jwt_customer_id`、`jwt_platform_admin`

`jwt` / `member_jwt` 的 `enabled: false` 时对应 `Token` 为 nil，中间件返回 503。

### 操作人上下文（InjectOperatorContext）

鉴权路由组在 `RequireJWT` **之后**挂载：

```go
admin.Use(httpx.RequireJWT(infra.JWT, "Authorization"))
admin.Use(httpx.InjectOperatorContext())
```

将 JWT 与请求元数据写入 `context.Context`，Application 使用 `contextx.OperatorFrom(ctx)`，**不依赖 `gin.Context`**。

推荐顺序：

```text
requestID → trace → RequireJWT → InjectOperatorContext → 业务 Handler
```

（`requestID` 与 `trace` 由 `httpx.NewEngine` 全局挂载。）

业务操作日志见 [audit-recorder.md](audit-recorder.md)。
...
- [业务操作日志（audit.Recorder）](audit-recorder.md)
