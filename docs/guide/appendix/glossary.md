...
| goeasy | 运行时 Go 模块，`github.com/txbao/goeasy` |
| goeasy-cli | 脚手架 CLI，安装后命令名为 `goeasy` |
| DDD Lite | 简化领域驱动设计：分层 + 实体行为，不强制事件溯源 |
| app_style | 应用层风格：`service`（默认）、`light_cqrs`、`full_cqrs` |
