...
GoEasy 选择第三种，并用 **goeasy** 承担运行时横切能力。
...
| 能力 | 传统手写 | GoEasy |
|------|----------|------|
| 新建服务 | 复制旧项目删代码 | `goeasy new` |
| 新模块 | 复制粘贴目录 | `goeasy add module` |
| API 契约 | 手写 proto | `goeasy add proto` |
| 运行时 | 自选库拼装 | 统一 `goeasy` P0–P4 |
...
| 框架探针 | `GET /healthz` | `goeasy/health`（配置开启） |
